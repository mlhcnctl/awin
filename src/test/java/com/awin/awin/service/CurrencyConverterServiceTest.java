...
public class CurrencyConverterServiceTest extends Helper {
    ...
    @Test
    public void make_url_get_all_rates_and_convert_currency() throws IOException {
        //given
        ...
        double convertedMoney = 0.825786;
        ...
        when(urlMaker.prepareUrlForListAllCurrencies(request)).thenReturn(urlForListAllCurrencies);
        when(urlMaker.prepareUrlForConvertCurrency(request)).thenReturn(urlForConvertCurrencies);
        when(exchangeRate.getAllRates(urlForListAllCurrencies)).thenReturn(object);
        when(exchangeRate.getConvertedMoney(urlForConvertCurrencies)).thenReturn(convertedMoney);

        //when
        Response res = currencyConverterService.convertSourceToTarget(request);
        ...
        verify(urlMaker).prepareUrlForListAllCurrencies(request);
        verify(urlMaker).prepareUrlForConvertCurrency(request);
